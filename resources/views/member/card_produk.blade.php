...
<div class="card shadow mb-2" style="border-radius: 10px;">
    ...
    {{-- <form id="favorit-form" action="{{ route('favorit.status',$p->id_produk) }}" method="POST"> --}}
        {{-- <button type="submit" class="btn fa fa-heart fa-2x fa-responsive cursor-pointer" style="position: absolute;top: 5%; left: 87%; transform: translate(-50%, -50%); -ms-transform: translate(-50%, -50%); background:transparent;"></button> --}}
    {{-- </form> --}}
    {{-- @if (!empty($p->foto_produk->getFirstMediaUrl())) --}}
        {{-- <img class="card-img-top cursor-pointer" src="{{$p->foto_produk->getFirstMediaUrl()}}" onclick="window.location.href='{{ route('detail.produk',$p->id_produk) }}'" style="height: 180px; border-radius: 10px 10px 0px 0px;" alt="Card image cap"/> --}}
    {{-- @else --}}
    <img class="card-img-top cursor-pointer"
        ...
        @endif
        onclick="window.location.href='{{ route('detail.produk',$p->id_produk) }}'" style="height: 180px; border-radius: 10px 10px 0px 0px;" alt="Terdapat Kesalahan Penampilan Foto"/>
    {{-- @endif --}}
