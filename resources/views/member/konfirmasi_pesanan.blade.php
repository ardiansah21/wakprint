                ...
                if (!empty($member->alamat['alamat'])){
                    for ($i = 0; $i < count($member->alamat['alamat']); $i++){
                        if ($member->alamat['alamat'][$i]['id'] === $member->alamat['IdAlamatUtama']){
                            $alamatPenerima = $member->alamat['alamat'][$i]['Alamat Jalan'].",".$member->alamat['alamat'][$i]['Kelurahan'].",".$member->alamat['alamat'][$i]['Kecamatan'].",".$member->alamat['alamat'][$i]['Kabupaten Kota'].",".$member->alamat['alamat'][$i]['Provinsi'].",".$member->alamat['alamat'][$i]['Kode Pos'];
                        }
                    }
                }
            @endphp
            <div class="row justify-content-end mb-5 mr-0">
                {{-- <form id="deleteKonfirmasiPesananForm" action="{{route('konfirmasi.pesanan.delete', $idPesanan)}}" method="POST" enctype="multipart/form-data"> --}}
                    {{-- @csrf --}}
                    <button id="batalkanBtn" class="btn btn-outline-danger-primary btn-lg text-primary-danger font-weight-bold mr-4"
                        style="border-radius:30px;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      font-size:24px;">
                        {{ __('Batalkan Pemesanan') }}
                    </button>
                {{-- </form> --}}
                <form action="{{route('konfirmasi.pesanan.update', $idPesanan)}}" method="POST" enctype="multipart/form-data">
                    @csrf
