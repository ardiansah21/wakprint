                ...
                <div class="row">
                            ...
                            <div class="list-group mb-4" style="border-radius: 10px;">
                                <a v-for="(pesanan, index) in pesanans" v-bind:key="index"
                                    ...
                                    <div class="d-flex w-10 justify-content-between">
                                        <label class="SemiBold mb-4" style="font-size: 18px;">ID Pesanan : @{{ pesanan . id }}</label>
                                        {{-- <small>3 days ago</small>
                                        --}}
                                        <span v-if="pesanan.count" class="badge badge-danger">@{{ pesanan . count }}</span>
                                    </div>
                                    <div class="row justify-content-left ml-0 mr-0">
                                        <label class="align-self-center mb-1 mr-2" style="font-size: 14px;">Percetakan :</label>
                                        <p class="SemiBold mb-2" style="font-size: 16px;">@{{ pesanan . nama_toko }}</p>
                                    </div>
                                    <label class="badge badge-sm text-white SemiBold py-2 px-2 mb-1 mr-2" style="background-color:#BC41BE; border-radius:8px; font-size: 12px;">@{{ pesanan . penerimaan }}</label>
                                    <label v-if="pesanan.status === 'Pending' || pesanan.status === 'Diproses'" class="badge badge-sm SemiBold py-2 px-2 mb-1" style="background-color: #FCFF82; border-radius:8px; color: black; font-size: 12px;">@{{ pesanan . status }}</label>
                                    <label v-else-if="pesanan.status === 'Selesai'" class="badge badge-sm SemiBold py-2 px-2 mb-1" style="background-color: #7BD6AF; border-radius:8px; color: white; font-size: 12px;">@{{ pesanan . status }}</label>
                                    <label v-else class="badge badge-sm SemiBold py-2 px-2 mb-1" style="background-color: #FF4949; border-radius:8px; color: white; font-size: 12px;">@{{ pesanan . status }}</label>
                    ...
                    <div class="col-md-8">
                        <div v-if="isActive != null" style="border: #EBD1EC 1px solid; border-radius: 10px;">
                            <div class="bg-light-purple px-4 py-3 mb-0" style="border-radius: 10px 10px 0px 0px;">
                                <div class="row  mb-0 ml-0">
                                    ...
                                    <div class="align-self-center ml-3">
                                        <label class="text-truncate align-middle SemiBold mb-0" style="font-size:18px; max-width:420px;">
                            ...
                            <div class="row justify-content-end py-3 px-2 ml-0 mr-0 mb-0" style="border-bottom: #EBD1EC 1px solid; border-radius: 0px;">
                                <a :href="'/konfirmasi-pembayaran/'+ pesanans[isActive].id_pesanan " class="SemiBold align-self-right py-2 px-4" style="border-radius: 10px; background-color: #7BD6AF; color:white; text-decoration:none;">Lihat Detail Pesanan</a>
                            </div>
                            ...
                            <form @submit.prevent="sendMessage" style="font-size:18px;">
                                <div class="row justify-content-between pl-2 mt-2 mb-0 ml-0 mr-0" style="background-color:#EBD1EC; border-top:#EBD1EC 1px solid; border-radius:0px 0px 10px 10px; width: 100%;">
                        ...
                        <div v-else class="text-center" style="border: #EBD1EC 1px solid; border-radius: 10px; height:600px;">
                            <label class="mt-5" style="font-size: 24px; color:#BC41BE;">Pilih Pesanan untuk Melihat Percakapan</label>
                        </div>
                    </div>
                </div>
            </div>
