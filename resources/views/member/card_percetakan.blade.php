<div class="card shadow mb-2" style="border-radius: 10px;">
    <a class="text-decoration-none" href="{{ route('detail.partner', $p->id_pengelola) }}" style="color: black;">
        @foreach ($atk as $a)
            @if ($a->id_pengelola === $a->partner->id_pengelola && $a->status === 'Tersedia')
                <label class="badge badge-pill badge-info bg-promo font-weight-bold align-bottom p-2 mb-1 mt-3"
                    style="position: absolute;top: 0%; left: 80%; font-size: 12px;">{{ __('ATK') }}</label>
            @else
                <label class="badge badge-pill badge-info bg-promo font-weight-bold align-bottom p-2 mb-1 mt-3"
                    style="position: absolute;top: 0%; left: 80%; font-size: 12px;" hidden>{{ __('ATK') }}</label>
            @endif
        @endforeach
    ...
    @else
        ...
        <div class="card-body">
            <label class="card-title text-truncate-multiline font-weight-bold"
                style="font-size: 24px; min-height:75px;">{{ $p->nama_toko }}</label>
            <label class="card-text text-truncate-multiline"
                style="font-size: 16px; min-height:55px;">{{ $p->alamat_toko }}</label>
            <label class="card-text text-sm text-truncate"
                style="font-size: 14px; min-height:10px; width:100%">{{ $p->deskripsi_toko ?? '-' }}</label>
        </div>
